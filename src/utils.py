...
class DataUtils:
    ...
    train_dataset = _dataset(_tmp_dir, train=True, download=True, transform=img_transform)
    test_dataset = _dataset(_tmp_dir, train=False, transform=img_transform)
      
    
class Trainer:
    def __init__(self, loader_kwargs):
        self._loader = torch.utils.data.DataLoader(DataUtils.train_dataset, **loader_kwargs)
        ...
        for batch_idx, (data, target) in enumerate(self._loader):
            ...
            if batch_idx % args.log_interval == 0:
                print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                    epoch, batch_idx * len(data), len(self._loader.dataset),
                    100. * batch_idx / len(self._loader), loss.item()))
                if args.dry_run:
                    break
...
class Tester:
    def __init__(self, loader_kwargs):
        self._loader = torch.utils.data.DataLoader(DataUtils.test_dataset, **loader_kwargs)
    ...
    def test(self, model, device):
        ...
        print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
            test_loss, correct, len(self._loader.dataset),
            100. * correct / len(self._loader.dataset)))
